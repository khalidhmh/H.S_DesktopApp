    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              أرسل رسالة فورية لجميع طلاب المبنى (مثال: تحذير انقطاع المياه)
            </p>
          </div>
        ...
        <div className="flex justify-end gap-3">
          <button className="px-6 py-2.5 bg-gray-100 text-gray-600 rounded-xl hover:bg-gray-200 font-bold">
            مسح
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            <TaskItem
              title="متابعة صيانة التكييف - غرفة 305"
              time="منذ ساعة"
              tag="عاجل"
              tagColor="bg-red-100 text-red-600"
            />
            <TaskItem
              title="التحقق من شكوى الطابق الثاني"
              time="منذ ساعتين"
              tag="مهم"
              tagColor="bg-yellow-100 text-yellow-700"
            />
            <TaskItem
              title="تأكيد حضور الطلاب الجدد"
              time="منذ 3 ساعات"
              tag="عادي"
              tagColor="bg-blue-100 text-blue-600"
            />
            <TaskItem
              title="فحص أجهزة الإنذار"
              time="منذ 4 ساعات"
              tag="عاجل"
              tagColor="bg-red-100 text-red-600"
            />
...
const StatCard = ({ title, value, subValue, icon: Icon, bg, text }: any) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between hover:shadow-md transition-shadow">
    <div>
      ...
      {subValue && (
        <span className="text-xs font-bold bg-gray-50 px-2 py-0.5 rounded text-gray-500 mt-1 inline-block">
          {subValue}
        </span>
      )}
    </div>
  ...
  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex justify-between items-center group hover:bg-white hover:shadow-sm transition-all">
    <div>
      <h4 className="font-bold text-primary flex items-center gap-2">
        {name}
        <span className="text-gray-400 text-xs font-normal bg-white px-2 py-1 border rounded-full">
          ({id})
        </span>
        {type === 'urgent' && (
...
const FloorCard = ({ number, status, color, icon: Icon, details }: any) => (
  <div
    className={`p-5 rounded-2xl border ${color} text-center flex flex-col items-center gap-2 shadow-sm bg-opacity-10 bg-white`}
  >
...
const ScheduleItem = ({ time, title, color = 'bg-white border-gray-100', active }: any) => (
  <div className="flex items-center gap-4 py-3 relative z-10">
    <div
      className={`w-16 py-1 text-center rounded-lg text-sm font-bold ${active ? 'bg-secondary text-primary shadow-md' : 'bg-gray-50 text-gray-500'}`}
    >
      {time}
    </div>
    <div
      className={`flex-1 p-3 rounded-xl border ${active ? 'bg-blue-50 border-blue-100' : color} ${active ? 'shadow-sm' : ''}`}
    >
      <h4 className={`font-bold ${active ? 'text-primary' : 'text-gray-700'}`}>{title}</h4>
    </div>
    {active && (
      <div className="absolute right-[61px] w-2.5 h-2.5 bg-secondary rounded-full border-2 border-white shadow-sm"></div>
    )}
  </div>
)
...
const TaskItem = ({ title, time, tag, tagColor }: any) => (
  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl hover:bg-white hover:shadow-md transition-all cursor-pointer border border-transparent hover:border-gray-100">
    ...
    <span className={`text-xs px-3 py-1 rounded-full font-bold ${tagColor}`}>{tag}</span>
  </div>
)
