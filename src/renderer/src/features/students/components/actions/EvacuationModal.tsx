...
export const EvacuationModal: React.FC<EvacuationModalProps> = ({
  ...
  const handleConfirm = async () => {
    ...
    const confirmed = window.confirm(
      `هل أنت متأكد من إخلاء طرف الطالب ${studentName}؟ هذا الإجراء لا يمكن التراجع عنه.`
    )
    if (!confirmed) return

    setIsSubmitting(true)
    try {
      // TODO: Implement actual evacuation logic via StudentService
      // This might involve updating student status to 'EVACUATED' or similar
      // await StudentService.evacuateStudent(studentId);

      console.log('Evacuating student:', studentId, studentName)

      // Simulate API call
      await new Promise((resolve) => setTimeout(resolve, 1000))

      alert(`تم إخلاء طرف الطالب ${studentName} بنجاح`)
      onClose()
    } catch (error) {
      console.error('Failed to evacuate student:', error)
      alert('فشل في إخلاء الطرف. حاول مرة أخرى.')
    } finally {
      ...
      >
        <div
          className={cn(
            ...
            checked ? 'translate-x-0' : '-translate-x-6' // RTL flip logic
          )}
        />
      </div>
    </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-6 flex items-start gap-3 text-yellow-800 text-sm">
            ...
            <p>
              لا يمكن إتمام عملية الإخلاء إلا بعد التأكد من استلام كافة المتعلقات وسلامة الغرفة.
            </p>
          </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex gap-3">
          ...
          >
            <LogOut size={18} />
            {isSubmitting ? 'جاري الإخلاء...' : 'تأكيد الإخلاء'}
          </button>
          <button
            onClick={onClose}
            disabled={isSubmitting}
            className="px-6 py-3 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-semibold rounded-lg transition-colors disabled:opacity-50"
          >
            إلغاء
          </button>
        </div>
