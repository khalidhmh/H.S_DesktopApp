...
export const ActivitiesList: React.FC<ActivitiesListProps> = ({ student }) => {
    ...
    const getIcon = (type: string) => {
        switch (type) {
            case 'sports': return <Trophy className="w-6 h-6 text-orange-500" />;
            case 'cultural': return <BookOpen className="w-6 h-6 text-purple-500" />;
    ...
    return (
        <div className="grid grid-cols-1 gap-4 text-right">
            {activities.map((activity, idx) => (
                <div key={idx} className="flex items-center p-4 bg-white rounded-lg border border-gray-100 hover:shadow-md transition-shadow">
                    <div className="p-3 bg-gray-50 rounded-full ml-4">
