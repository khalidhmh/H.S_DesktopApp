...
interface ComplaintState {
  ...
  reportComplaint: (data: { title: string; description: string; priority: string; studentId: number }) => Promise<void>;
  reportMaintenance: (data: { type: string; description: string; priority: string; roomId?: number; location?: string }) => Promise<void>;
...
export const useComplaintStore = create<ComplaintState>((set, get) => ({
  ...
  fetchIssues: async (filter) => {
      ...
      set({ isLoading: true });
      try {
          const issues = await ComplaintService.getAllIssues(currentFilter === 'ALL' ? undefined : currentFilter);
