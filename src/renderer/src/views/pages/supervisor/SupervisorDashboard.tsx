    ...
    return (
        <div className="space-y-8" dir="rtl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Attendance Status */}
                <Card className={`border-l-4 ${isAttendanceTaken ? 'border-l-green-500' : 'border-l-red-500'}`}>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">حالة التمام اليومي</CardTitle>
                        <ClipboardCheck className={`h-4 w-4 ${isAttendanceTaken ? 'text-green-500' : 'text-red-500'}`} />
                    </CardHeader>
                    <CardContent>
                                    ...
                                    <div key={complaint.id} className="p-4 flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-muted-foreground">الطالب: {complaint.studentName} - {new Date(complaint.createdAt).toLocaleDateString('ar-EG')}</p>
                                            </div>
                                        </div>
