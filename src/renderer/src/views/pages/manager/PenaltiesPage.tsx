                ...
                <CardContent className="p-0">
                    <Table>
                        ...
                        <TableBody>
                            {isLoading ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="h-24 text-center">جاري تحميل البيانات...</TableCell>
                                </TableRow>
                                ...
                                filteredPenalties.map((penalty) => (
                                    <TableRow key={penalty.id} className="group hover:bg-red-50/10 transition-colors">
                                        <TableCell className="font-semibold text-gray-700">{(penalty as any).student?.name || 'غير معروف'}</TableCell>
                                        <TableCell className="font-mono text-sm text-gray-500">{(penalty as any).student?.universityId || '—'}</TableCell>
                                        <TableCell>
...
function PenaltyTypeBadge({ type }: { type: string }) {
    switch (type) {
        case 'WARNING':
            return <Badge variant="outline" className="border-yellow-200 bg-yellow-50 text-yellow-700 hover:bg-yellow-100">تنبيه شفهي</Badge>;
        case 'WRITTEN_WARNING':
            return <Badge variant="outline" className="border-orange-200 bg-orange-50 text-orange-700 hover:bg-orange-100">إنذار كتابي</Badge>;
        case 'FINE':
            return <Badge variant="destructive" className="bg-red-100 text-red-700 hover:bg-red-200 border-transparent">غرامة مالية</Badge>;
        case 'DISMISSAL':
            return <Badge variant="destructive" className="px-3">فصل نهائي</Badge>;
        default:
