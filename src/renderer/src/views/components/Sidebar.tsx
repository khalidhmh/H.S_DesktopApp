...
export const Sidebar = () => {
  ...
  // الروابط المشتركة بين الجميع
  const commonLinks = [
    { name: 'الطلاب', path: '/students', icon: Users },
    { name: 'الغرف', path: '/rooms', icon: BedDouble },
    { name: 'العهدة', path: '/inventory', icon: Package },
  ];

  const managerLinks = [
    { name: 'لوحة التحكم', path: '/', icon: LayoutDashboard },
    ...commonLinks, // إضافة المشترك
    { name: 'أرشيف التمام', path: '/attendance-history', icon: Archive },
    { name: 'الشكاوى', path: '/complaints', icon: MessageSquareWarning },
    { name: 'الصيانة', path: '/maintenance', icon: Wrench },
    { name: 'الإشعارات', path: '/notifications', icon: Bell },
    { name: 'التعاميم', path: '/memos', icon: Megaphone },
  ...
  const supervisorLinks = [
    { name: 'لوحة التحكم', path: '/', icon: LayoutDashboard }, // يذهب لداشبورد المشرف
    ...commonLinks, // إضافة المشترك (الآن المشرف يرى الطلاب والغرف والعهدة)
    { name: 'التمام اليومي', path: '/attendance', icon: ClipboardCheck },
    { name: 'الشكاوى', path: '/complaints', icon: MessageSquareWarning },
    { name: 'الصيانة', path: '/maintenance', icon: Wrench },
    { name: 'الإشعارات', path: '/notifications', icon: Bell },
    { name: 'التقارير', path: '/reports', icon: FileText }, // المشرف يحتاج تقارير أيضاً
    { name: 'الإعدادات', path: '/settings', icon: Settings }, // المشرف له إعدادات خاصة
  ];
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="overflow-hidden">
            ...
            <p className="text-xs text-gray-400 truncate">
              {role === 'MANAGER' ? 'مدير النظام' : 'مشرف مبنى'}
            </p>
          </div>
        </div>
        <button
