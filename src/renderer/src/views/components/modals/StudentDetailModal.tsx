...
export function StudentDetailModal({ student, isOpen, onClose }: StudentDetailModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[700px] p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-white/20 shadow-2xl rounded-3xl" dir="rtl">
                ...
                <div className="px-6 pb-6 -mt-12 relative z-10">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <Badge variant={student.status === 'ACTIVE' ? 'success' : 'secondary'} className="mt-14 text-sm px-4 py-1">
                    ...
                    <div className="space-y-1 mb-6">
                        ...
                        <DialogDescription className="text-gray-500 font-medium">{student.universityId}</DialogDescription>
                    </div>
                    ...
                    <Tabs defaultValue="info" className="w-full">
                        <TabsList className="grid w-full grid-cols-3 mb-6">
                            ...
                            <TabsTrigger value="penalties" className="text-red-600 data-[state=active]:text-red-700 data-[state=active]:bg-red-50">المخالفات والعقوبات</TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="info" className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-4">
                                    ...
                                    <InfoItem icon={Building} label="السكن" value={student.room ? `${student.room.building} - غرفة ${student.room.roomNumber}` : 'غير مسكن'} />
                                ...
                                <div className="space-y-4">
                                    ...
                                    <InfoItem icon={Phone} label="ولي الأمر" value={student.guardianContact || 'غير متوفر'} dir="ltr" className="text-red-500" />
                        ...
                        <TabsContent value="penalties">
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <Button variant={isAdding ? "ghost" : "destructive"} size="sm" onClick={() => setIsAdding(!isAdding)}>
                                ...
                                {isAdding && (
                                    <div className="bg-red-50 p-4 rounded-lg border border-red-100 space-y-3 animate-in fade-in slide-in-from-top-2">
                                        <div className="space-y-2">
                                            ...
                                            <Select value={newPenalty.type} onValueChange={(v) => setNewPenalty({ ...newPenalty, type: v })}>
                                        ...
                                        <Button className="w-full bg-red-600 hover:bg-red-700 text-white" onClick={handleIssuePenalty}>
                                            تأكيد المخالفة
                                        </Button>
                                    </div>
                                )}
                                ...
                                <div className="border rounded-md overflow-hidden">
                                    <Table>
                                        ...
                                        <TableBody>
                                            {isLoading ? (
                                                <TableRow>
                                                    <TableCell colSpan={3} className="text-center py-4">جاري التحميل...</TableCell>
                                                </TableRow>
                                            ) : penalties.length === 0 ? (
                                                <TableRow>
                                                    <TableCell colSpan={3} className="text-center py-8 text-muted-foreground">لا يوجد مخالفات مسجلة 👏</TableCell>
                                                </TableRow>
                                            ) : (
                                                penalties.map((p) => (
                                                    <TableRow key={p.id}>
                                                        <TableCell>{format(new Date(p.date), 'dd/MM/yyyy', { locale: ar })}</TableCell>
                                                        <TableCell>
                                                            <Badge variant={p.type === 'WARNING' ? 'outline' : 'destructive'}>
                                                                {p.type === 'WARNING' ? 'تنبيه' :
                                                                    p.type === 'WRITTEN_WARNING' ? 'إنذار' :
                                                                        p.type === 'FINE' ? 'غرامة' : 'فصل'}
                                                            </Badge>
                                                        </TableCell>
                                                        <TableCell className="font-medium text-sm text-gray-700">{p.reason}</TableCell>
                                                    </TableRow>
                                                ))
                                            )}
                                        </TableBody>
    ...
    return (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-md transition-all group">
            ...
            <div>
                ...
                <p className={`text-sm font-semibold text-gray-700 ${className}`} dir={dir}>{value}</p>
            </div>
        </div>
    );
}
